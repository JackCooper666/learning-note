...
# 前缀积
前缀积是指数组中从起始位置到某个位置的所有元素的乘积。类似于前缀和的概念，但使用的是乘法运算而非加法。

## 定义

对于一个长度为 n 的数组 nums，它的前缀积数组 prefixProd 定义为：
- `prefixProd[i] = nums[0] × nums[1] × ... × nums[i]`
特别地：
- `prefixProd[0] = nums[0]` (第一个元素的前缀积就是它本身)
- `prefixProd[n-1]` 是整个数组所有元素的乘积

```cpp
def prefix_product(nums):
    n = len(nums)
    prefix = [1] * n
    prefix[0] = nums[0]  # 第一个元素
...
def suffix_product(nums):
    n = len(nums)
    suffix = [1] * n
    suffix[-1] = nums[-1]  # 最后一个元素
    
    for i in range(n-2, -1, -1):  # 从倒数第二个元素开始向前遍历
